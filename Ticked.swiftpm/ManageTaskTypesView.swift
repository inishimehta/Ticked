...
struct ManageTaskTypesView: View {
    ...
    @State private var taskTypes: [TaskType] = TaskStore.loadTaskTypes()
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 10) {
                ...
                Button {
                    ...
                    let newType = TaskType(name: trimmed)
                    taskTypes.insert(newType, at: 0)
                    TaskStore.saveTaskTypes(taskTypes)  // ✅ persist
                    newTypeName = ""
                } label: {
                ...
                ForEach(taskTypes) { type in
                    HStack(spacing: 12) {
                        ...
                        Button(role: .destructive) {
                            if let index = taskTypes.firstIndex(where: { $0.id == type.id }) {
